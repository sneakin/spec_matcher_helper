#
# ActiveRecord association matchers for RSpec
# "Stolen" from http://snippets.dzone.com/posts/show/4721
#
module AssociationMatchers
  class AssociationReflection
    ...
    def through(model)
      @expected_options[:through] = model
      self
    end
  end

  matcher :belong_to => AssociationReflection do
    def initialize(name)
  ...
  matcher :has_one => AssociationReflection do
    def initialize(name)
      super :has_one, name
    end
  ...
  matcher :has_many => AssociationReflection do
    def initialize(name)
      super :has_many, name
    end
  ...
  matcher :have_and_belong_to_many => AssociationReflection do
    def initialize(name)
